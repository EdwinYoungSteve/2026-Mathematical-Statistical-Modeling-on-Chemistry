...
const MODEL_METRICS = [
  { key: 'Stacking', name: 'Stacking 集成', auc: .7921, r2: .1923, color: '#43d5bf' },
  { key: 'Voting', name: 'Voting 集成', auc: .7845, r2: .1885, color: '#7db5ff' },
  { key: 'RF', name: 'Random Forest', auc: .7712, r2: .1812, color: '#f7ad69' },
  { key: 'ET', name: 'Extra Trees', auc: .7656, r2: .1778, color: '#b38cff' },
  { key: 'HGBT', name: 'HistGradientBoosting', auc: .7598, r2: .1735, color: '#ef8f8f' },
  { key: 'SVM', name: 'SVM (RBF)', auc: .7481, r2: .1633, color: '#6bd5db' },
  { key: 'MLP', name: 'MLP 神经网络', auc: .7289, r2: .1421, color: '#849cf0' }
];
...
function normalizeRow(raw, index = 1) {
  return {
    ...
    Pred_Active: toNumber(getField(raw, ['Pred_Active', 'Pred_Label']), 0),
    Prob_Active: toNumber(getField(raw, ['Prob_Active', 'Pred_Prob_Active', 'probability']), 0)
  };
}
...
function renderOverview() {
  ...
  $('#page-overview').innerHTML = `
    ...
    <div class="metrics-grid">
      <div class="metric-card"><span class="metric-label">候选分子</span><strong class="metric-value">${rows.length.toLocaleString()}</strong><span class="metric-foot"><span class="up">↑ 12.4%</span> 较上次筛选</span></div>
      ...
      <div class="metric-card"><span class="metric-label">最佳模型 AUC</span><strong class="metric-value teal">0.792</strong><span class="metric-foot"><span class="up">↑ 3.8%</span> Stacking 集成</span></div>
    </div>
    <div class="content-grid">
      <section class="panel"><div class="panel-header"><div><h2 class="panel-title">预测分布</h2><p class="panel-subtitle">pIC50 预测值与活性概率分布</p></div><span class="chip">${state.dataSource === 'demo' ? '演示数据' : '已导入数据'}</span></div><div class="panel-body"><div class="chart-wrap"><canvas id="distributionChart"></canvas></div><div class="legend"><span>预测 pIC50</span><span>活性阈值 6.0</span></div></div></section>
      <section class="panel"><div class="panel-header"><div><h2 class="panel-title">高潜力分子</h2><p class="panel-subtitle">按预测活性概率排序</p></div><button class="panel-link" data-go="batch">查看全部 →</button></div><div class="panel-body"><div class="ranking-list">${rows.slice(0, 5).map((row, index) => `<div class="ranking-row"><span class="ranking-rank">0${index + 1}</span><span class="ranking-name" title="${escapeHtml(row.SMILES)}">${escapeHtml(row.ChEMBL_ID)}</span><span class="ranking-score">${number(row.Prob_Active * 100, 1)}%</span><div class="progress"><span style="width:${clamp(row.Prob_Active * 100, 2, 100)}%"></span></div></div>`).join('')}</div></div></section>
    </div>
    <section class="panel table-panel"><div class="panel-header"><div><h2 class="panel-title">最近筛选结果</h2><p class="panel-subtitle">${state.dataSource === 'demo' ? '来自数据演示/虚拟筛选结果.csv' : '当前导入文件'} · 共 ${rows.length} 条</p></div><button class="panel-link" data-go="batch">打开批量筛选 →</button></div>${renderDataTable(rows.slice(0, 7), false)}</section>`;
  $$('#page-overview [data-go]').forEach((button) => { button.onclick = () => setPage(button.dataset.go); });
  $('#overviewRefresh').onclick = () => { renderOverview(); drawOverviewChart(); showToast('已刷新当前项目数据'); };
...
function renderSingle() {
  ...
  $('#page-single').innerHTML = `
    <div class="page-heading"><div><h1>单分子预测</h1><p>输入标准 SMILES，快速估计 pIC50、活性概率和关键分子属性。</p></div><div class="heading-actions"><button class="button" id="useDemoMolecule">使用示例分子</button></div></div>
    <div class="single-grid"><section class="panel form-panel"><form id="singleForm"><label class="form-label" for="smilesInput">SMILES 分子式</label><textarea class="textarea" id="smilesInput" required placeholder="例如：COc1ccc2[nH]c(C(=O)NCCCN3CCOCC3)cc2c1">${escapeHtml(result?.smiles || '')}</textarea><p class="input-hint">支持标准 SMILES 字符串。演示模式下使用分子组成估算；连接后端 API 后将调用训练好的 Stacking / Voting 模型。</p><div class="form-row"><div><label class="form-label" for="modelSelect">预测模型</label><select class="select" id="modelSelect"><option value="Stacking">Stacking 集成（推荐）</option><option value="Voting">Voting 集成</option><option value="RF">Random Forest</option><option value="HGBT">HistGradientBoosting</option></select></div><div><label class="form-label" for="thresholdInput">活性阈值</label><input class="text-input" id="thresholdInput" type="number" min="0" max="12" step="0.1" value="6.0" /></div></div><div class="form-actions"><button type="reset" class="button" id="clearSingle">清空</button><button type="submit" class="button primary" id="predictButton"><span class="button-icon">✦</span> 开始预测</button></div></form></section>
...
async function predict(smiles, model, threshold) {
  try {
    ...
    const response = await fetch('/api/predict', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ smiles, model, threshold }), signal: controller.signal }); clearTimeout(timer);
    if (response.ok) { const payload = await response.json(); return normalizeApiResult(payload, smiles, model, threshold); }
...
function renderBatch() {
  const allRows = state.batchRows; const filtered = allRows.filter((row) => row.Prob_Active >= state.threshold); const pages = Math.max(1, Math.ceil(filtered.length / state.batchPageSize)); state.batchPage = clamp(state.batchPage, 1, pages); const visible = filtered.slice((state.batchPage - 1) * state.batchPageSize, state.batchPage * state.batchPageSize);
  $('#page-batch').innerHTML = `<div class="page-heading"><div><h1>批量虚拟筛选</h1><p>导入候选分子列表，使用活性概率阈值快速定位优先验证对象。</p></div><div class="heading-actions"><button class="button" id="downloadTemplate">↓ 下载模板</button><button class="button primary" id="exportResults">导出筛选结果</button></div></div><div class="batch-hero"><section class="panel dropzone" id="dropzone"><h3>导入候选分子</h3><p>上传包含 SMILES、ChEMBL_ID 的 CSV 文件。系统会识别已有预测列，或在接入 API 后批量计算。</p><label class="dropzone-label" for="csvInput">＋ 选择 CSV 文件</label><input id="csvInput" type="file" accept=".csv,text/csv" /><span id="fileName" style="display:block;margin-top:10px;color:#64808a;font-size:9px;">当前：${state.dataSource === 'demo' ? '虚拟筛选结果.csv（演示）' : '已导入文件'}</span></section><section class="panel batch-options"><span class="form-label">筛选条件</span><div class="range-line"><span>活性概率不低于</span><b id="thresholdValue">${number(state.threshold * 100, 0)}%</b></div><input class="range" id="probabilityRange" type="range" min="0" max="1" step=".05" value="${state.threshold}" /><div class="range-line"><span>命中分子</span><b>${filtered.length} / ${allRows.length}</b></div><div class="progress" style="margin-top:10px;"><span style="width:${allRows.length ? filtered.length / allRows.length * 100 : 0}%"></span></div></section></div><section class="panel table-panel"><div class="batch-summary"><div><strong>筛选结果</strong><span>按活性概率降序排列</span></div><div class="table-actions"><button class="button" id="sortByPic50">按 pIC50</button><button class="button" id="sortByProb">按概率</button></div></div>${renderDataTable(visible, true)}<div class="pagination">${Array.from({ length: pages }, (_, index) => `<button class="page-btn ${index + 1 === state.batchPage ? 'active' : ''}" data-batch-page="${index + 1}">${index + 1}</button>`).join('')}</div></section>`;
  ...
  $('#sortByPic50').onclick = () => { state.batchRows.sort((a, b) => b.Pred_pIC50 - a.Pred_pIC50); renderBatch(); };
  $('#sortByProb').onclick = () => { state.batchRows.sort((a, b) => b.Prob_Active - a.Prob_Active); renderBatch(); };
  $('#exportResults').onclick = () => exportRows(filtered, 'molsight_screening_results.csv');
  $('#downloadTemplate').onclick = () => exportRows([{ ChEMBL_ID: 'EXAMPLE_001', SMILES: 'CCOc1ccc2[nH]c(C(=O)NCC)cc2c1' }], 'molsight_template.csv');
  const selectAll = $('#selectAll'); if (selectAll) selectAll.onchange = () => $$('.row-check').forEach((checkbox) => { checkbox.checked = selectAll.checked; });
}

function loadCsvFile(file) {
  const reader = new FileReader(); reader.onload = () => { const rows = parseCsv(reader.result); if (!rows.length) { showToast('未识别到有效的 CSV 数据'); return; } state.batchRows = rows; state.rows = rows; state.batchPage = 1; state.dataSource = 'upload'; renderBatch(); showToast(`已导入 ${rows.length} 条分子记录`); }; reader.onerror = () => showToast('文件读取失败，请重试'); reader.readAsText(file, 'UTF-8');
}
...
function renderEvaluation() {
  $('#page-evaluation').innerHTML = `<div class="page-heading"><div><h1>模型评估</h1><p>对比回归与分类任务表现，确认当前集成模型是否满足虚拟筛选目标。</p></div><div class="heading-actions"><button class="button" id="openReport">查看优化报告</button></div></div><div class="evaluation-grid"><section class="panel"><div class="panel-header"><div><h2 class="panel-title">分类模型对比 · ROC-AUC</h2><p class="panel-subtitle">测试集表现，越高越好</p></div><span class="chip">测试集 n=100</span></div><div class="model-bars">${MODEL_METRICS.map((metric) => `<div class="model-bar"><label>${metric.name}</label><div class="bar-track"><span style="width:${metric.auc * 100}%;background:${metric.color}"></span></div><b>${metric.auc.toFixed(3)}</b></div>`).join('')}</div><div class="model-note"><b>推荐：</b>Stacking 集成 AUC 0.7921，在当前测试集上综合表现最佳；Voting 集成作为更快的候选方案。</div></section><section class="panel"><div class="panel-header"><div><h2 class="panel-title">回归任务指标</h2><p class="panel-subtitle">pIC50 数值预测</p></div></div><div class="metric-large"><div class="detail-box"><span>最佳 R²</span><b class="score-good">0.1923</b><small>Stacking</small></div><div class="detail-box"><span>RMSE</span><b>1.374</b><small>测试集</small></div><div class="detail-box"><span>MAE</span><b>1.081</b><small>测试集</small></div><div class="detail-box"><span>交叉验证</span><b>5-fold</b><small>分层抽样</small></div></div></section></div><div class="content-grid section-spaced"><section class="panel"><div class="panel-header"><div><h2 class="panel-title">模型选择建议</h2><p class="panel-subtitle">针对当前 EGFR 活性筛选任务</p></div></div><div class="panel-body"><div class="interpretation" style="margin-top:0;"><b>用于初筛：</b>优先使用 Stacking，适合需要较高召回率的候选物排序。<br/><br/><b>用于快速迭代：</b>Voting 集成推理链路更轻量，适合在化合物库扩展阶段批量运行。<br/><br/><b>注意：</b>指标来自现有项目报告，正式实验前请使用独立外部验证集复核。</div></div></section><section class="panel"><div class="panel-header"><div><h2 class="panel-title">现有可视化</h2><p class="panel-subtitle">项目已生成的评估图</p></div></div><div class="panel-body" style="display:grid;grid-template-columns:1fr 1fr;gap:9px;"><a href="../环节六：模型优化/figures/O1_regression_before_after.png" target="_blank" class="report-card" style="min-height:90px;padding:11px;text-decoration:none;"><span class="report-icon">◒</span><h3 style="margin-top:7px;">回归优化前后</h3></a><a href="../环节六：模型优化/figures/O3_roc_all_optimized.png" target="_blank" class="report-card" style="min-height:90px;padding:11px;text-decoration:none;"><span class="report-icon">⌁</span><h3 style="margin-top:7px;">ROC 曲线</h3></a></div></section></div>`;
  $('#openReport').onclick = () => setPage('reports');
}

function renderReports() {
  $('#page-reports').innerHTML = `<div class="page-heading"><div><h1>实验报告</h1><p>快速打开项目各阶段生成的报告、图表与虚拟筛选结果。</p></div><div class="heading-actions"><button class="button primary" data-go="batch">进入筛选结果</button></div></div><div class="report-list"><article class="report-card"><span class="report-icon">◫</span><h3>模型优化报告</h3><p>超参数调优、特征筛选和集成模型结论。</p><a href="../环节六：模型优化/优化报告.txt" target="_blank">打开报告 →</a></article><article class="report-card"><span class="report-icon">◌</span><h3>虚拟筛选结果</h3><p>按活性概率排序的候选分子清单。</p><a href="../数据演示/虚拟筛选结果.csv" target="_blank">打开 CSV →</a></article><article class="report-card"><span class="report-icon">▤</span><h3>集成学习评估</h3><p>Stacking / Voting 与基模型性能对比。</p><a href="../环节五：集成学习/集成学习评估报告.txt" target="_blank">打开报告 →</a></article><article class="report-card"><span class="report-icon">◒</span><h3>模型训练评估</h3><p>分类、回归指标及交叉验证结果。</p><a href="../环节四：模型训练/模型评估报告.txt" target="_blank">打开报告 →</a></article><article class="report-card"><span class="report-icon">⌁</span><h3>筛选可视化</h3><p>瀑布图、富集曲线和预测误差分析。</p><a href="../数据演示/figures/D5_enrichment_curve.png" target="_blank">查看图表 →</a></article><article class="report-card"><span class="report-icon">⌘</span><h3>数据清洗报告</h3><p>原始数据清洗、缺失值与异常值处理记录。</p><a href="../环节二：数据清洗与EDA/清洗报告.txt" target="_blank">打开报告 →</a></article></div><section class="panel section-spaced"><div class="panel-header"><div><h2 class="panel-title">使用说明</h2><p class="panel-subtitle">前端运行与后端接入</p></div></div><div class="panel-body"><div class="interpretation" style="margin-top:0;"><b>直接运行：</b>使用任意静态服务器打开 frontend/index.html，例如 VS Code Live Server。直接双击打开也可使用单分子演示和文件导入功能。<br/><br/><b>接入真实模型：</b>提供 POST /api/predict 接口，接收 { smiles, model, threshold }，返回 { prediction, probability, confidence, mw, aromatic, hetero, rings, logp, uncertainty }，页面会自动切换为真实模型结果。</div></div></section>`;
...
async function loadDemoData() {
  try {
    ...
    const rows = parseCsv(await response.text()); if (rows.length && state.dataSource === 'demo') { state.rows = rows; state.batchRows = [...rows]; renderPage(state.page); }
...
renderPage('overview');
loadDemoData();
window.addEventListener('resize', () => { if (state.page === 'overview') drawOverviewChart(); });
